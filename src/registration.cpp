...
class Registrator {
 public:
  ...
  ros::NodeHandle nhp_;
  ros::Subscriber sub;
  Registrator(ros::NodeHandle nh, ros::NodeHandle nhp) : init(false), acc(new Cloud), nh_(nh), nhp_(nhp) {
    // Get parent and child IDs for the TF
    nhp_.param("parent", parent, string());
    nhp_.param("child", child, string());

    // Setup ROS Publishers and subscriber
    pub = nh_.advertise<Cloud>("/output", 1);
    sub = nh_.subscribe("/input", 1, &Registrator::callback, this);
  }
 private:
  string parent, child;
  Cloud::Ptr acc;
